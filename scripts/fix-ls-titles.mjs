#!/usr/bin/env node
/**
 * Fix lifesongs hymn titles to include number prefix
 * e.g. "荣耀荣耀归于父神" -> "001荣耀荣耀归于父神"
 *
 * Usage: node scripts/fix-ls-titles.mjs [--dry-run]
 */

import COS from 'cos-nodejs-sdk-v5';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DRY_RUN = process.argv.includes('--dry-run');

const envPath = path.resolve(__dirname, '..', '.env');
const env = {};
for (const line of fs.readFileSync(envPath, 'utf-8').split('\n')) {
  const m = line.match(/^(\w+)=(.*)$/);
  if (m) env[m[1]] = m[2].trim();
}

const BUCKET = 'clover-1313238640';
const REGION = 'ap-nanjing';
const cos = new COS({ SecretId: env.COS_SECRET_ID, SecretKey: env.COS_SECRET_KEY });

function cosGet(key) {
  return new Promise((resolve, reject) => {
    cos.getObject({ Bucket: BUCKET, Region: REGION, Key: key }, (err, data) => {
      if (err) reject(err);
      else resolve(data.Body.toString('utf-8'));
    });
  });
}

function cosPut(key, content, contentType) {
  return new Promise((resolve, reject) => {
    cos.putObject({
      Bucket: BUCKET, Region: REGION, Key: key,
      Body: Buffer.from(content, 'utf-8'),
      ContentType: contentType,
    }, (err, data) => {
      if (err) reject(err);
      else resolve(data);
    });
  });
}

async function main() {
  const raw = await cosGet('manifest.json');
  const manifest = JSON.parse(raw);

  let updated = 0;

  for (const hymn of manifest.hymns) {
    if (!hymn.id.startsWith('ls-')) continue;

    const numStr = hymn.id.replace('ls-', '');
    const padded = numStr.padStart(3, '0');

    // Check if title already starts with the number
    if (hymn.title.startsWith(padded)) continue;

    // Add number prefix
    hymn.title = padded + hymn.title;
    updated++;
  }

  console.log(`Updated ${updated} titles`);

  if (DRY_RUN) {
    const lsHymns = manifest.hymns.filter(h => h.id.startsWith('ls-'));
    console.log(`Sample:`);
    for (const h of lsHymns.slice(0, 5)) {
      console.log(`  ${h.id}: ${h.title}`);
    }
    return;
  }

  await cosPut('manifest.json', JSON.stringify(manifest, null, 2), 'application/json');
  console.log('Manifest uploaded');
}

main().catch(e => { console.error(e); process.exit(1); });
